    ...
    <div class="invoice-box">
        <table cellpadding="0" cellspacing="0">
                    ...
                    <table>
                        <tr>
                            ...
                            <td>
                                ...
                                {{ $nama->name }}<br>
                                {{ $nama->phone_number }}<br>
                                {{ $nama->address }} 
                                {{ $nama->district->name }}, {{ $nama->district->city->name }}<br>
                                {{ $order->district->city->province->name }}
                            </td>
                        </tr>
                    </table>
            ...
            <tr class="item">
                <td>
                    {{ $row->product->name }}<br>
                    <strong>Harga</strong>: Rp {{ number_format($row->price) }} x {{ $row->qty }} + {{ $order->cost }}
                </td>
                <td>Rp {{ number_format($row->price * $row->qty + $order->cost) }}</td>
            </tr>
            @endforeach
            ...
            <tr class="total">
                <td></td>
                <td>
                    Subtotal: Rp {{ number_format($order->subtotal + $order->cost) }}
                </td>
            </tr>
