...
<body>
    ...
    <div class="container grid-container" id="top-content">
        ...
        <div class="content-1">
            <div class="card">
                <img src="{{ asset('/gambar/' . $ban->gambar) }}">
                <div class="card-img-overlay text-white d-flex flex-column">
                    <h2 class="card-title">{{$ban->title}}</h2>
                    <a href="/education/{{$ban->slug}}" class="stretched-link"><span>{{$ban->user->name}}
                        </span><span id="date">{{$ban->created_at->format('d M Y')}}</span></a>
                </div>
            </div>
        </div>
        @endforeach
    ...
    <div class="container-fluid" id="content">
        ...
        <div class="card-deck">
            ...
            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6" id="col-card">
                <a href="/education/{{$item->slug}}">
                    <div class="card" id="content-card">
                        ...
                        <div class="card-body">
                            <h3 class="card-title">{{ Illuminate\Support\Str::limit($item->title, 20) }}</h3>
                            <div class="description d-flex justify-content-between">
                                <div class="author"><span><i class="fas fa-user"></i> {{$item->user->name}}</span></div>
                                <div class="date-view text-right">
                                    <span class="mr-3"><i class="far fa-clock"></i>
                                        {{$item->created_at->format('d M Y')}}</span>
                                    <span><i class="fas fa-eye"></i>
                                        {{views($item)->count()}}</span></div>
                            </div>
                        </div>
                    </div>
                </a>
            ...
            @empty
            <h3 class="m-auto">No Recent Posts</h3>
            @endforelse
        </div>
        {{ $post->links() }}
    </div>
    ...
    <div class="container-fluid" id="content">
        ...
        <div class="card-deck">
            @forelse ($blog as $row)
            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6" id="col-card">
                <a href="/education/{{$row->slug}}">
                    <div class="card" id="content-card">
                        <img src="{{ asset('/gambar/' . $row->gambar) }}" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h3 class="card-title">{{ Illuminate\Support\Str::limit($row->title, 20) }}</h3>
                            <div class="description d-flex justify-content-between">
                                <div class="author"><span><i class="fas fa-user"></i> {{$row->user->name}}</span></div>
                                <div class="date-view text-right">
                                    <span class="mr-3"><i class="far fa-clock"></i>
                                        {{$row->created_at->format('d M Y')}}</span>
                                    <span><i class="fas fa-eye"></i>
                                        {{views($row)->count()}}</span></div>
                            </div>
                        </div>
                    </div>
                </a>
